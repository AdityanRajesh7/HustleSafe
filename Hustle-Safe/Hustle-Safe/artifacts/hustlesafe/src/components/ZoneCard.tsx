...
import { Users, CloudRain, ShieldAlert, AlertTriangle } from "lucide-react";
  ...
  return (
    <motion.button
      ...
      className={cn(
        ...
        selected ? "ring-2 ring-primary border-transparent shadow-md" : "border-border hover:shadow-md hover:border-primary/30",
    ...
    >
      <div className="flex justify-between items-start w-full">
        ...
        <span className={cn("px-2 py-1 rounded-full text-[10px] font-bold uppercase flex items-center gap-1.5", statusColor)}>
      ...
      <div className="flex items-end justify-between w-full">
        <div>
          <div className="text-xs text-muted-foreground mb-1 font-medium">GDS SCORE</div>
          <div className={cn(
            "text-5xl font-display font-bold leading-none tracking-tighter",
            zone.gds_score >= 80 ? "text-foreground" :
            zone.gds_score >= 60 ? "text-destructive" :
            zone.gds_score >= 40 ? "text-warning" : "text-success"
          )}>
            {zone.gds_score}
          </div>
        </div>
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground w-full pt-4 border-t border-border/50">
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-semibold text-foreground">{zone.active_workers}</span>
        </div>
